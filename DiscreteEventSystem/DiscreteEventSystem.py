##
# Echelon Base Stock version of the DES
# With an order cap at store level (this is optional)
# Includes an allocation policy for online demand and stores
# Not optimised. For an optimised version, see DiscreteEventSystemSpeedy.py
##
...
class DES:
    ...
    def __init__(self, periods, N, store_lead_time, warehouse_lead_time, production_capacity, warehouse_capacity, store_capacity, p, cost_dfw, holding_warehouse, holding_store, shortage_warehouse, shortage_store, warehouse_initial, store_initial):
        ...
        # Always log the data
        self.log = {'StartingInvWarehouse': [], 'StartingInventoryStore': [], 'Order': [],'Allocation_Req':[], 'PostOrderWarehouse': [], 'PostOrderStore': [], 'Demand': [], 'DFW_Fulfillment': [], 'DFW_total': [], 'PostDemandWarehouse': [], 'PostDemandStore': [], 'EndingInventoryWarehouse': [],'EndingInventoryStore': [],'PeriodCost': []}
    ...
    def calc_store_orders(self,t):
        ...
        if total_orders <= self.x_warehouse[t][0]:
            ...
            # Here we do not have the stock to fulfil the store orders
            # We need to allocate the stock
            # Stock is assigned to maximizes the minimum among stores of the total of current inventory and inventory on route to a store
    ...
    def calc_warehouse_orders(self,t):
        ...
        inv_pipeline_all = np.sum(self.x_warehouse[t])+np.sum([np.sum(self.x_store[store][t]) for store in range(self.N)])
    ...
    def allocate_stock(self, desired_orders, available_stock, t):
        ...
        desired_orders = [sp.poisson(self.demand[t][0]).ppf(self.cu_w/(self.cu_w+self.co_w))] + desired_orders
    ...
    def run(self, demand):
        ...
        for t in range(self.T):
            ...
            si_w = self.x_warehouse[t].copy()
            si_s = self.x_store[:,t].copy()
            self.log['StartingInvWarehouse'].append(si_w)
            self.log['StartingInventoryStore'].append(si_s)
            ...
            po_x = self.x_store[:,t].copy()
            self.log['PostOrderStore'].append(po_x)

           
            self.x_warehouse[t][0] -= np.sum(Q_stores)
            po_w = self.x_warehouse[t].copy() 
            self.log['PostOrderWarehouse'].append(po_w)
            ...
            #self.period_cost[t][0] += self.x_warehouse[t][1]*self.co_w
            ...
            for store in range(self.N):
                ...
                # Add storage cost for newly arrived stock?
                #self.period_cost[t][store+1] += self.x_store[store][t][1]*self.co_s
                # Add dfw
            ...
            self.log['Order'].append([Q_wh] + Q_stores)
            ...
            self.log['PostDemandWarehouse'].append(self.x_warehouse[t])
            self.log['PostDemandStore'].append(self.x_store[:,t])
            self.log['EndingInventoryWarehouse'].append(self.x_warehouse[t+1].copy())
            self.log['EndingInventoryStore'].append(self.x_store[:,t+1].copy())
